...
export function buildRoute(
  ...
  const templateKeys = Array.from(route.matchAll(/\{([^}]+)\}/g)).map(
    match => match[1]! // <-- Fix: assert capture always exists
  );
  ...
  for (const key of templateKeys) {
    if (!(key in params)) {
      console.warn(
        `[buildRoute] Warning: Route expects parameter "${key}" but it was not provided.`
      );
    }
  }
  ...
  for (const key in params) {
    ...
    if (value === undefined) {
      console.warn(
        `[buildRoute] Warning: Parameter "${key}" is undefined and will not be replaced.`
      );
      continue;
    }
